...
7. **反模式合集**（可选）
   - 用 ✗/✓ 对照列出**使用本 skill 时**容易出错的方式
   - 借鉴依据：LLM 对反例敏感度高于正例——"不要 Y，因为 Z"比"应该 X"对行为约束更强
   - 适合 worker 类 skill（产物清晰、容易出错的场景）；纯参考文档 / 流程编排 skill 可省略
   - 位置：放在"验证方式"之后、"不覆盖范围"之前
   - 每条格式：`✗ 反模式名（具体场景） / 为什么错 / ✓ 正确做法`
   - 数量建议：3-5 条，覆盖最常见的失守模式
   - 反模式必须是**使用本 skill 时**的失守，不是 skill 本身要检测的代码层反模式（除非 skill 就是 review 类）
...
**结构检查**：
- [ ] 包含所有必需小节（"反模式合集"和"覆盖声明"可选；其余必需）
- [ ] 使用中文小节标题
- [ ] 小节顺序符合标准（1-10）
- [ ] 若包含"反模式合集"：放在"验证方式"之后、"不覆盖范围"之前；每条用 ✗/✓ 对照格式
