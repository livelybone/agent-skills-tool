...
1. **检查实现前提**
   ...
   - 确认本次要消费的测试真实存在、覆盖当前 spec 主路径与每个声明的功能域，且带 `Scenario ID` 与 `@scenario` / `@spec-ref` 最小追溯
   - 若测试缺少 `Scenario ID`、`@scenario` 或 `@spec-ref`，停止实现并回退到 `test-design-and-implementation`
...
3. **锁定实现范围**
   ...
   - 对照测试中的 `@scenario` / `@spec-ref` 与模型中的 `upstream-ref`，确认每类能力的约束来源
...
- [ ] 当前消费的测试带 `Scenario ID` 与 `@scenario` / `@spec-ref` 最小追溯；若缺失，已停止并回退上游
