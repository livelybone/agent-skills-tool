...
| 跨 agent 审查 Spec     | Decision Log 条目                  | 当前会话（受下方"存放位置"例外条款约束）           |
| Scenario 生成          | `<module>.scenarios.md`            | `spec/`            |
| 跨 agent 审查 Scenario | Decision Log 条目                  | 当前会话（受下方"存放位置"例外条款约束）           |
| Test Implementation    | 测试文件（按 repo-structure 规则） | `src/` 或 `tests/` |
| 跨 agent 审查 Test     | Decision Log 条目                  | 当前会话（受下方"存放位置"例外条款约束）           |
...
**Decision Log 和 Decision Report 是会话产物，禁止落盘。**

- ✅ 允许：在会话上下文中维护 Decision Log 条目，流程结束后在会话内输出 Decision Report
- ❌ 禁止：写入任何磁盘文件，包括但不限于：
  - `spec/<epic>.decision-log.md`
  - `spec/<module>.decision-log.md`
  - `spec/<epic>.decision-report.md`
  - `docs/`、`memory/`、临时目录下的等价文件

理由：

1. Decision Log 是流程内部状态，不是交付物；Decision Report 的消费者是当前会话的用户，不是未来的读者
2. 落盘会产生维护负担（PR review、版本化、清理）且与"会话产物"定位矛盾
3. 如需跨会话追溯，走 git log / PR 描述，不走 spec/ 目录

**例外**：仅当仓库级 CLAUDE.md **同时满足以下全部条件**时，方可落盘：
...
skill 本身不产生落盘行为。任何未同时满足上述三项的仓库级表述，一律按"禁止落盘"处理。
...
> ⚠️ **存放位置**：仅维护在当前会话上下文中，禁止落盘。详见上文"Decision Log / Decision Report 的存放位置（硬性约束）"。
...
> ⚠️ **存放位置**：仅在会话中输出给用户，禁止落盘为 `spec/*.decision-report.md` 等文件。详见上文"Decision Log / Decision Report 的存放位置（硬性约束）"。
