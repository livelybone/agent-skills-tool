...
- **实现细节耦合**：断言依赖内部状态存放位置、私有字段、具体代码路径（而非输入→输出/副作用的行为契约）
- **琐碎断言**：验证 getter/setter、简单赋值、类型转换等无业务风险的行为
- **脆弱快照**：使用 snapshot 匹配完整输出结构，内部重构即会失败
- **冗余测试**：多个 test case 验证同一行为契约，仅输入值不同且不构成边界
- **私有函数测试**：直接 import 并测试未导出的内部 helper

**判断标准**：如果一次纯内部重构（不改变外部行为）导致该测试失败，说明测试耦合了实现，应标记为问题。
