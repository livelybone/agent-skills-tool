...
3. 实现完成后，baseline 列表交由 CI 验证（步骤 11.1 Baseline 对比）
...
- 实现当前 Spec 范围内的所有功能域，测试验证交由 CI 验证（步骤 11）
- 在改变行为之前暴露 Spec 歧义
...
实现过程中可以运行测试获取反馈，遇到失败时按以下流程处理（最终通过验证由 CI 验证步骤 11 负责）：

1. **实现有误** → 修改实现代码
2. **测试本身有误** → 停止，向人工报告，回溯到 Scenario 层确认后再改测试
3. **需求变更导致测试过时** → 走完整流程：Model（增量更新）→ Spec → Scenario（人工审批）→ 更新测试 → 再实现
