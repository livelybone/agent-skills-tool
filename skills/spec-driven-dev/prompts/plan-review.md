...
- Epic 建模（`epic-model.md`）——必须提供，审查参照
- Plan（`plan.md`）
...
1. **聚合边界完整性**：epic-model.md中**每一个**聚合（`Aggregate.X` 锚点）是否有且仅有一个模块在"持有聚合"字段中承载？
   - 有 & 唯一 → 通过
   - 多个模块持有同一聚合 → 标注 `[Critical][聚合被切散]`
   - 无模块持有某聚合 → 标注 `[Major][聚合未落位]`
2. **upstream-ref 合法性**：Plan 中所有"持有聚合/模块依赖/产出契约"字段的 `upstream-ref` 是否都指向 epic-model.md中真实存在的锚点？
   - 是 → 通过
   - 虚假引用 → 标注 `[Critical][虚假上游引用]`
3. **契约可追溯性**：每条"模块依赖"和"产出契约"是否都能在 epic-model.md的跨聚合关系（`Rel.<A>-<B>` 锚点）中找到依据？
   - 是 → 通过
   - 凭空新造的跨模块依赖 → 标注 `[Major][越界契约]`，要求先回修 epic-model.md或删除越界契约
