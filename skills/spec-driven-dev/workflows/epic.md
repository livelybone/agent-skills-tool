...
- 触发规则：Auto 模式强制；Standard 模式只有在 `Review Skip Policy = complexity-allowed` 时，才按 `guides/complexity.md` 的表格「步骤 4 Modeling Review」行判断是否可跳过
...
| 8 | Spec Review（M） | 按 `guides/review.md` 的 `spec-review` 配置执行；Auto 或 `Review Skip Policy = never` 时强制，Standard + `complexity-allowed` 时按模块复杂度字段（见 plan）决定是否可跳过 |
...
| 10 | Test Review（M） | 按 `guides/review.md` 的 `test-review` 配置执行；Auto 或 `Review Skip Policy = never` 时强制，Standard + `complexity-allowed` 时按模块复杂度决定是否可跳过 |
...
| 12 | Implementation Review（M） | 按 `guides/review.md` 的 `impl-review` 配置执行；Auto 或 `Review Skip Policy = never` 时强制，Standard + `complexity-allowed` 时按模块复杂度决定是否可跳过 |
...
- **Standard 模式**：
  - `Review Skip Policy = never` 时，步骤 4 / 6 / 8 / 10 / 12 全部强制执行
  - `Review Skip Policy = complexity-allowed` 时，步骤 4 分两种情形——若走正常建模，按 `guides/complexity.md`「步骤 4 Modeling Review」行判断是否执行；若走 `modeling_exemption`，**任意复杂度均强制执行** Exemption Review（不受 Epic 复杂度字段影响）
  - 步骤 6：任意复杂度均强制（Epic 的 Plan Review 是编排骨架，无法跳过）
  - `Review Skip Policy = complexity-allowed` 时，步骤 8 / 10 / 12 按 plan 中该模块的复杂度字段 + `guides/complexity.md` 对应行决定是否执行
