...
- 触发规则：Auto 模式强制；Standard 模式按 `guides/complexity.md` 的表格「步骤 4 Modeling Review」行判断是否可跳过；`Review Runner Policy = manual-only` 时不得启动跨 agent runner
...
| 8 | Spec Review（M） | 按 `guides/review.md` 的 `spec-review` 配置执行；Auto 强制，Standard 按模块复杂度字段（见 plan）决定是否可跳过 |
...
| 10 | Test Review（M） | 按 `guides/review.md` 的 `test-review` 配置执行；Auto 强制，Standard 按模块复杂度决定是否可跳过 |
...
| 12 | Implementation Review（M） | 按 `guides/review.md` 的 `impl-review` 配置执行；Auto 强制，Standard 按模块复杂度决定是否可跳过 |
...
- **Standard 模式**：
  ...
  - `Review Runner Policy = manual-only` 时不得启动跨 agent runner；遇到步骤 6、建模豁免审查等强制 Review 时必须阻塞或升级
