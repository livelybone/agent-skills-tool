...
- **跨模块共享不变量**
- **稳定锚点**：每条实体/关系/不变量/派生关系/聚合/共享不变量必须带显式锚点（六类命名空间和格式见 `guides/upstream-ref.md`），供下游标注 `upstream-ref`
...
- 回修必须明确记录 `upstream-change-log.md`（存放在 `spec/` 目录，与 `plan.md` 同级）。格式：
  ...
  - **触发模块**：<发现问题的模块名>
  - **修改内容**：<epic-model.md 中具体修改了什么（锚点/聚合/关系）>
  - **原因**：<为什么需要修改>
  - **影响范围**：<哪些模块的哪些产物需要失效/重做>
  ```

### 非 Epic 模式不适用

单模块需求不涉及 `epic-model.md`，本规则不适用；模块内部的建模迭代遵循普通 Spec 迭代规则（见 `guides/iteration-rules.md`）。
