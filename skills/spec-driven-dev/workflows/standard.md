...
- Standard 的 Review 跳过行为由 Intake 阶段确认的 `Review Skip Policy` 决定。
- `Review Skip Policy = never` 时，Standard 也必须执行 Review stage，不得记录 `skipped`。
- `Review Skip Policy = complexity-allowed` 时，Standard 才允许按 `guides/complexity.md` 合法跳过部分独立 Review runner。
...
Review stage 的 runner、task-name、prompt、失败回退和 checkpoint 更新规则见 `guides/review.md`。Standard 只在 `Review Skip Policy = complexity-allowed` 时额外允许按 `guides/complexity.md` 合法跳过部分独立 runner。

若 `Review Skip Policy = never`，Review runner 不可用、缺少授权或执行失败都必须记录为 `blocked:<reason>` 或升级给用户，不得改写为 `skipped`。
...
- `模式` 字段写 `standard`。
- Intake 阶段必须记录用户选择的 `Review Skip Policy`；用户未明确回答时记录为 `never`。
