...
本 guide 只补充 `SKILL.md` 中 Workflow Verification 阶段的 upstream coverage 校验规则，不定义 workflow 顺序或 worker 内容语义。
...
本文档定义上游覆盖矩阵（Upstream Coverage Matrix）的结构和机械校验规则。`upstream-ref` 的语法、锚点命名、N/A 规则、各阶段落位方式见 `upstream-ref.md`（唯一定义点）。
...
| `domain/<name>.md` Derivation Chains | 每条派生链必须有测试：输入根变量，断言派生值等于等式结果 |
...
参考实现脚本：`$SPEC_DRIVEN_DEV_SKILL_DIR/scripts/check-upstream-coverage.sh`

```bash
bash "$SPEC_DRIVEN_DEV_SKILL_DIR/scripts/check-upstream-coverage.sh" \
  --upstream docs/models/domain/order.md \
  --matrix docs/coverage/order-coverage.md \
  --refs-glob 'tests/**/*.test.ts,docs/scenarios/**/*.md'

# 回归测试（在修改脚本后 / CI 里常跑）：
bash "$SPEC_DRIVEN_DEV_SKILL_DIR/scripts/check-upstream-coverage.sh" --self-test
```
...
1. 运行 `$SPEC_DRIVEN_DEV_SKILL_DIR/scripts/check-plan-structure.sh --plan <plan.md> --upstream <Epic 涉及的所有聚合来源单元>`。
2. 针对 Plan 实际引用的单元运行 `$SPEC_DRIVEN_DEV_SKILL_DIR/scripts/check-upstream-coverage.sh`，校验 Plan 引用。
3. 按建模单元分别运行 `$SPEC_DRIVEN_DEV_SKILL_DIR/scripts/check-upstream-coverage.sh`，校验 Spec/Test/Impl 覆盖。

自动化封装应留在具体项目中；本 guide 只定义必需调用，不维护某个仓库专属的 shell loop。
