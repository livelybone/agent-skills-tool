...
spec-driven-dev 的各阶段产物（Plan / Spec / Test / Impl）必须**可追溯到建模文件**（`modeling-first` v0.3+ 产出的 `docs/models/<scenario>/<name>.md`）。
...
实现完成后（见 `prompts/feature-implementation.md` 的 Spec 完整性 + Upstream Coverage 校验步骤）。
...
Auto 模式下，每次 upstream 回修（见 `workflows/epic.md` 的"迭代回流规则"）必须记入 `upstream-change-log.md`；Upstream Coverage Matrix 也必须随之更新。
