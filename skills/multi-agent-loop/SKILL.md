...
9. **有界循环**：每一轮在执行层面独立——agent 不感知上一轮的存在，也不读取上一轮的产物。但 controller 的**调度决策**依赖本轮裁决结果来判断是否继续。
   ...
   - **终止条件**（满足任一即停止）：
     - 达到最大 3 轮（`run_agent.sh` 拒绝命名尾部为 `-r<N>$` 且 N>3 的 agent 任务，无 override flag。gate 针对默认命名阻断无意识滑坡——主动改名或直接调用内部 `--prepared-status-file` 可绕过，但那一步本身即是显式自我欺骗，不是 gate 要防的无意识合理化。正当做法是在 controller 层调整策略：拆分 scope、降低单轮深度、或 escalate 给用户）
