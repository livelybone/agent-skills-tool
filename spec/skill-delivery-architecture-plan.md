...
- **边界**：消费完整需求或 `ClarifiedRequirement`，调用 `modeling-first` 完成建模，在 Epic 场景生成 plan，并产出 handoff packet
...
- **边界**：把完整需求和 `spec-driven-dev` 的 handoff packet 翻译成技术文档，不直接生成测试或代码
- **模块依赖**：`spec-driven-dev` 的 requirement baseline + models + optional plan + optional review notes handoff (upstream-ref: docs/models/domain/skill-delivery-architecture.md#Rel.SpecDrivenDev-TechSpecWriting)
...
| tech-spec-writing | — | pending | 下一步实现 |
